...
async def check_user(uid=None, msg=None, restricted=False) -> bool:
    ...
    # ၁။ Admin ဖြစ်ပါက အမြဲတမ်း သုံးခွင့်ပြုမည်
    ...
    # ၂။ Group Access Logic (Bot ကို Public ဖွင့်ထားသော်လည်း Group မှလွဲ၍ Private Chat တွင် Admin မှတစ်ပါး မည်သူမျှ သုံးခွင့်မပြုပါ)
    if msg and msg.chat:
        ...
        if is_group:
            if bot_set.bot_public:
                ...
                if user_id in all_chats or msg.chat.id in all_chats:
                    return True
        else:
            # Group မဟုတ်ပါက (Private Chat/DM ဖြစ်ပါက) Admin မှလွဲ၍ အခြားသူများကို သုံးခွင့်မပြုပါ
            return False

    # Fallback (msg မပါလာပါက စစ်ဆေးရန် logic)
    if bot_set.bot_public:
        return True
    else:
        all_chats = list(bot_set.admins) + bot_set.auth_chats + bot_set.auth_users
